...
(import "" "init" (func $init (param i32)))
...
(global $pl-count (mut i32) (i32.const 0))
(global $pl-channel (mut i32) (i32.const 0))
...
(func $start
  ...
  (call $init (global.get $num-channels))
  ...
  (loop $loop
    ...
    (i32.store8 offset=12 (local.get $j) (local.get $fine-tune))
    (i32.store8 offset=13 (local.get $j) (local.get $volume))
  ...
  (loop $channel
    (i32.store8 offset=36 (local.get $channel)
      (i32.div_u
        (i32.sub
          (local.get $channel)
          (i32.const 0x275))
        (i32.const 52)))
    (br_if $channel (i32.lt_u (local.tee $channel (i32.add (local.get $channel) (i32.const 52)))
...
(func $sequence-tick
  ...
  (if (i32.le_s (global.get $tick) (i32.const 0))
    (then
      ...
      (loop $channel
        ...
        (if (i32.eq (local.get $effect) (i32.const 0xE))
          (then
            (local.set $effect
              (i32.or (i32.const 0x10) (i32.shr_u (local.get $param) (i32.const 4))))
            (local.set $param (i32.and (local.get $param) (i32.const 0xf)))))

        ;; Convert effect 0 to effect 0xE
        (if (i32.and (i32.eqz (local.get $effect))
                     (i32.gt_s (local.get $param) (i32.const 0)))
          (then
        ...
        (if (i32.eqz (i32.and (i32.eq (local.get $effect) (i32.const 0x1d))
                              (i32.gt_s (local.get $param) (i32.const 0))))
          (then
        ...
        block $1e
          ...
          (if (i32.le_s (local.get $param) (i32.const 0))
            (then
            ...
            (select
              ...
              (local.tee $volume
                (i32.sub
                  ...
                  (local.get $param)))
              (i32.lt_s (local.get $volume) (i32.const 0))))
          br $X
            ...
            (select
              ...
              (local.tee $volume
                (i32.add
                  (i32.load8_u offset=33 (local.get $channel))
                  (local.get $param)))
              (i32.gt_s (local.get $volume) (i32.const 64))))
          br $X
        end $17
          unreachable
        end $16 ;; pattern loop
          ...
          (if (i32.and
                (i32.lt_s (i32.load8_u offset=38 (local.get $channel)) (global.get $row))
                (i32.lt_s (global.get $break-pattern) (i32.const 0)))
            (then
              ...
              (if (i32.eq (global.get $pl-channel) (i32.load8_u offset=36 (local.get $channel)))
            ...
            (select
              ...
              (local.tee $period
                (i32.sub
                  ...
                  (local.get $param)))
              (i32.lt_s (local.get $period) (i32.const 0))))
          br $X
        end $f  ;; set speed
          (if (i32.gt_s (local.get $param) (i32.const 0))
            (then
              (if (i32.lt_s (local.get $param) (i32.const 32))
                ...
                (else
                  (call $set-tempo (local.get $param))))))
          br $X
        end $d  ;; pattern break
          (if (i32.lt_s (global.get $pl-count) (i32.const 0))
            (then
              ...
              (if (i32.ge_s (global.get $next-row) (i32.const 64))
                (then
                  (global.set $next-row (i32.const 0))))))
          br $X
        end $c  ;; set volume
          (i32.store8 offset=33 (local.get $channel)
            (select
              (i32.const 64)
              (local.get $param)
              (i32.gt_s (local.get $param) (i32.const 64))))
          br $X
        end $b
          (if (i32.lt_s (global.get $pl-count) (i32.const 0))
            (then
          ...
          (call $vibrato (local.get $channel))
          br $X
        end $4  ;; vibrato
          (if (i32.gt_s
                ...
                (i32.const 0))
            (then
              (i32.store8 offset=42 (local.get $channel) (local.get $param'))))
          (if (i32.gt_s
                (local.tee $param' (i32.and (local.get $param) (i32.const 0xf)))
                (i32.const 0))
            (then
        ...
        (call $update-frequency (local.get $channel))
        (br_if $channel (i32.lt_u (local.tee $channel (i32.add (local.get $channel) (i32.const 52)))
                                (local.get $channel-end)))))
    (else
      (loop $channel
        ...
        block $1d
          ...
          (if (i32.eq (local.get $param) (local.get $fx-count))
            (then
              (call $trigger (local.get $channel))))
          br $X
        end $1c
          ...
          (if (i32.ge_s (local.get $fx-count) (local.get $param))
            (then
              (i32.store8 offset=39 (local.get $channel) (i32.const 0))
            ...
            (select
              ...
              (local.tee $period
                (i32.add
                  (local.get $period)
                  (local.get $param)))
              (i32.gt_u (local.get $period) (i32.const 65535))))
          br $X
        end $1 ;; portamento up
          (i32.store16 offset=24 (local.get $channel)
            (select
        ...
        (if (i32.gt_s (local.get $effect) (i32.const 0))
          (then
            (call $update-frequency (local.get $channel))))
...
(func $vibrato (param $channel i32)
...
(func $waveform (param $phase i32) (param $type i32) (result i32)
  (local $amp i32)
  block $done
  block $0
  block $1
  block $2
  block $3
    (br_table $0 $1 $2 $3 (i32.and (local.get $type) (i32.const 3)))
  end $3  ;; Random
    (local.set $amp
...
(func $trigger (param $channel i32)
  ...
  (if (i32.and
    ...
    (then
      ...
      (if (i32.and
            ...
            (i32.gt_s (i32.load (local.get $channel)) (i32.const 0)))
        (then
  ...
  ;; key is non-zezro
  (if (i32.gt_s
        (local.tee $key (i32.load16_u offset=28 (local.get $channel)))
        (i32.const 0))
    (then
...
(func $update-frequency (param $channel i32)
  (local $period i32)
  (local $volume i32)
  ...
  (f32.store offset=16 (local.get $channel)
    (f32.div
      (f32.convert_i32_u
        (i32.div_u
          (i32.mul (global.get $c2-rate) (i32.const 428))
          (local.get $period)))
  ...
  (loop $loop
    ...
    (local.set $remain
      (select
        ...
        (i32.gt_s (local.get $remain) (local.get $count))))
    ...
    (loop $channel
      ...
      ;; TODO: chan->mute, chan->panning
      (local.set $left-ampl (f32.load offset=20 (local.get $channel)))
      (local.set $right-ampl (local.get $left-ampl))
      (local.set $sample-data (i32.load (local.get $instrument-ptr)))

      (block $exit-buffer
        (br_if $exit-buffer (i32.eqz (local.get $instrument-ptr)))
        ...
        (loop $buffer
          ...
          (if $exit-if (f32.ge (local.get $sample-idx) (local.get $loop-end))
            (then
              ...
              ;; TODO: Use fmod here?
              (br_if $exit-if (f32.lt (local.get $sample-idx) (local.get $loop-end)))
              (loop $loop
          ...
          (loop $sample
            ...
            (br_if $sample (f32.lt (local.tee $sample-idx (f32.add (local.get $sample-idx) (local.get $sample-step)))
